...
class Casualty:
    def __init__(self, idx):
        self.id = idx
        self.survival_without_care = np.random.uniform(0.1, 0.5)
        self.required_blood_units = np.random.randint(0, 3)
        self.required_care_time = np.random.uniform(0.1, 0.3)  # In hours
        self.survival_with_care = np.random.uniform(0.6, 0.9)
...
# Initialize casualties list
casualties = [Casualty(i) for i in range(NUM_CASUALTIES)]
